...
const EditProfile = () => {
    ...
    const { register, handleSubmit, formState: { errors }, setValue } = useForm<FormData>({
        resolver: joiResolver(EditUserSchema), // Asegúrate de actualizar esto también
        mode: "onChange",
    });
    ...
    useEffect(() => {
        if (user) {
            ...
            console.warn("No user data available in Redux.");
        }
    }, [user, setValue]);
    ...
    const onSubmit = async (data: FormData) => {
        ...
        console.log("Data to be sent:", data); // Imprimir datos a enviar
        ...
        <div className="flex flex-col items-center justify-start gap-10 px-4 m-auto" style={{ background: `linear-gradient(#ff9846, #ffffff)` }}>
            <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-md p-6 md:max-w-2xl">
                ...
                <div className="flex flex-col md:flex-row md:space-x-2">
                    <TextInput {...register("name.first")} placeholder="First Name" className="mb-2" color={errors.name?.first ? "failure" : "gray"} />
                    {errors.name?.first && <p className="text-red-600">{errors.name.first.message}</p>}
                    <TextInput {...register("name.middle")} placeholder="Middle Name" className="mb-2" />
                    <TextInput {...register("name.last")} placeholder="Last Name" className="mb-2" color={errors.name?.last ? "failure" : "gray"} />
                    {errors.name?.last && <p className="text-red-600">{errors.name.last.message}</p>}
                </div>
                <TextInput {...register("phone")} placeholder="Phone" type="tel" className="mb-2" color={errors.phone ? "failure" : "gray"} />
                {errors.phone && <p className="text-red-600">{errors.phone.message}</p>}

                <TextInput {...register("image.url")} placeholder="Image URL" className="mb-2" color={errors.image?.url ? "failure" : "gray"} />
                {errors.image?.url && <p className="text-red-600">{errors.image.url.message}</p>}
                <TextInput {...register("address.country")} placeholder="Country" className="mb-2" color={errors.address?.country ? "failure" : "gray"} />
                {errors.address?.country && <p className="text-red-600">{errors.address.country.message}</p>}
                <TextInput {...register("address.city")} placeholder="Address City" className="mb-2" color={errors.address?.city ? "failure" : "gray"} />
                {errors.address?.city && <p className="text-red-600">{errors.address.city.message}</p>}
                <TextInput {...register("address.street")} placeholder="Street" className="mb-2" color={errors.address?.street ? "failure" : "gray"} />
                {errors.address?.street && <p className="text-red-600">{errors.address.street.message}</p>}
                <TextInput {...register("address.houseNumber")} placeholder="House Number" type="number" className="mb-2" color={errors.address?.houseNumber ? "failure" : "gray"} />
                {errors.address?.houseNumber && <p className="text-red-600">{errors.address.houseNumber.message}</p>}
                <TextInput {...register("address.zip")} placeholder="Zip Code" type="number" className="mb-2" color={errors.address?.zip ? "failure" : "gray"} />
